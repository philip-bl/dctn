...
my_merged_cores = (
  ...
  oe.contract(
    my_cores[0], ("b0", "b1", "in0"),
    my_cores[1], ("b1", "b2", "in1"),
    ("b0", "b2", "in0", "in1"),
  ),
  oe.contract(
    my_cores[2], ("b2", "b3", "in2"),
    my_cores[3], ("b3", "b4", "in3"),
    ("b2", "b4", "in2", "in3"),
  ),
  oe.contract(
    my_cores[4], ("out", "b4", "b5"),
    my_cores[5], ("b5", "b6", "in4"),
    ("out", "b4", "b6", "in4")
  ),
  oe.contract(
    my_cores[6], ("b6", "b7", "in5"),
    my_cores[7], ("b7", "b8", "in6"),
    ("b6", "b8", "in5", "in6"),
  ),
  oe.contract(
    my_cores[8], ("b8", "b9", "in7"),
    my_cores[9], ("b9", "b10", "in8"),
    ("b8", "b10", "in7", "in8"),
  )
)
...
processed_cores = (
  *mpo.linear_region.module_list[0](input[:, :4]).tensor.squeeze(0).split(1), # batch × lbond × rbond
  mpo.linear_region.module_list[1](input[:, 4]).tensor, # batch × out × lbond × rbond
  *mpo.linear_region.module_list[2](input[:, 5:]).tensor.squeeze(0).split(1), # batch × lbond × rbond
)
my_processed_cores = (
  oe.contract(
    my_merged_cores[0], ("b0", "b2", "in0", "in1"),
    input[:, 0], ("batch", "in0"),
    input[:, 1], ("batch", "in1"),
    ("batch", "b0", "b2")
  ...
  oe.contract(
    my_merged_cores[1], ("b2", "b4", "in2", "in3"),
    input[:, 2], ("batch", "in2"),
    input[:, 3], ("batch", "in3"),
    ("batch", "b2", "b4")
  ),
  oe.contract(
    my_merged_cores[2], ("out", "b4", "b6", "in4"),
    input[:, 4], ("batch", "in4"),
    ("batch", "out", "b4", "b6")
  ),
  oe.contract(
    my_merged_cores[3], ("b6", "b8", "in5", "in6"),
    input[:, 5], ("batch", "in5"),
    input[:, 6], ("batch", "in6"),
    ("batch", "b6", "b8")
  ),
  oe.contract(
    my_merged_cores[4], ("b8", "b10", "in7", "in8"),
    input[:, 7], ("batch", "in7"),
    input[:, 8], ("batch", "in8"),
    ("batch", "b8", "b10")
  ),
...
my_output = oe.contract(
  torch.tensor([1.0] + [0.0]*10), ("b0",),
  my_processed_cores[0], ("batch", "b0", "b2"),
  my_processed_cores[1], ("batch", "b2", "b4"),
  my_processed_cores[2], ("batch", "out", "b4", "b6"),
  my_processed_cores[3], ("batch", "b6", "b8"),
  my_processed_cores[4], ("batch", "b8", "b10"),
  torch.tensor([1.0] + [0.0]*10), ("b10",),
  ("batch", "out")
)
...
explicit_tensor_output = oe.contract(
  explicit_tensor, ("out", 'in0', 'in1', 'in2', 'in3', 'in4', 'in5', 'in6', 'in7', 'in8'),
