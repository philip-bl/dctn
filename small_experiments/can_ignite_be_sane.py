...
dataset = FashionMNIST(DATASET_PATH, transform=Compose((ToTensor(), Normalize((0.286,), (0.353,)))))
TRAIN_SIZE = 50000
BATCH_SIZE = 512
DEVICE = torch.device("cuda")
train_dataloader = DataLoader(dataset, BATCH_SIZE, sampler=SubsetRandomSampler(range(TRAIN_SIZE)),
                              pin_memory=(DEVICE.type == "cuda"), drop_last=True)
val_dataloader = DataLoader(dataset, BATCH_SIZE, sampler=SubsetRandomSampler(range(TRAIN_SIZE, 60000)),
...
evaluator = create_supervised_evaluator(model, {"loss": ignite.metrics.Loss(F.cross_entropy)}, DEVICE, prepare_batch=prepare_batch)
def foo(_):
