...
class EPSPlusLinear(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.linear = nn.Linear(
            hparams.out_size * (28 - hparams.kernel_size + 1) ** 2, 10, bias=True
        )
    ...
    def forward(self, batch: torch.Tensor) -> torch.Tensor:
        ...
        concatenated = torch.cat(
            (sin_squared.unsqueeze(3), cos_squared.unsqueeze(3)), dim=3
        ).unsqueeze(
            0
        )  # 1×batch×height×width×2
    ...
    def train_dataloader(self):
        return DataLoader(
            Subset(
                self.dataset_creator(self.hparams.dataset_root, transform=ToTensor()),
                range(50000),
            ),
    ...
    def validation_epoch_end(self, outputs):
        non_reduced_losses = (
            record["loss"].item() * record["batch_size"] for record in outputs
        )
    ...
    def test_epoch_end(self, outputs):
        ...
        return {
            ...
            "progress_bar": {
                ...
                "test_accuracy": outputs["accuracy"],
            },
        }
