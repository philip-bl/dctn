...
save_where = (
    "/mnt/important/experiments/tiny_mnist_multilinear_classifier_adam_nonnegative.pth"
)
...
for i in range(num_iters):
    ...
    train_ce_loss_avg = (
        train_ce_loss.item()
        if i == 0
        else train_ce_loss_avg * mov_avg_coeff  # noqa: F821
        + train_ce_loss.item() * (1 - mov_avg_coeff)
    )
    train_acc = (logits.argmax(dim=1) == y_batch).sum().float().item() / y_batch.shape[0]
    train_acc_avg = (
        train_acc
        if i == 0
        else train_acc_avg * mov_avg_coeff + train_acc * (1 - mov_avg_coeff)  # noqa: F821
    )
    print(
