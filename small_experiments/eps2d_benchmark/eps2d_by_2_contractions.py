...
def create_tensors(device, dtype):
    ...
    inputs = [
        torch.randn((64, 25, 25, 2), device=device, dtype=dtype, requires_grad=True)
        for _ in range(16)
    ]
    return (param, *inputs)
...
def do_the_thing(*args):
    param = args[0]
    inputs = args[1:]
    input_part_0 = torch.einsum(
        "bhwi,bhwj,bhwk,bhwl,bhwm,bhwn,bhwo,bhwp->bhwijklmnop", inputs[:8]
    )
    input_part_1 = torch.einsum(
        "bhwi,bhwj,bhwk,bhwl,bhwm,bhwn,bhwo,bhwp->bhwijklmnop", inputs[8:]
    )
...
print(
    benchmark_torch(do_the_thing_oe, create_tensors, torch.float32, torch.device("cuda"), 500)
)
