...
DIR = "/mnt/important/experiments/2_epses_plus_linear_fashionmnist/dropout_rate_finding/"
...
d = {"dropout_p": [], "best_vacc": [], "best_vacc_nitd": []}
for subdir in subdirs:
    d["dropout_p"].append(get_dropout_p(subdir))
    best_vacc_nitd, best_vacc = get_best_vacc_info(subdir)
    d["best_vacc_nitd"].append(best_vacc_nitd)
    d["best_vacc"].append(best_vacc)

df = pd.DataFrame.from_dict(d).set_index("dropout_p").sort_index()
