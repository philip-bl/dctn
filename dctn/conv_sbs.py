...
class ConvSBS(nn.Module):
    ...
    def init_khrulkov_normal(self, std_of_elements_of_matrix: Optional[float] = None) -> None:
        ...
        if std_of_elements_of_matrix is not None:
            ...
            var_of_elements_of_matrix = 2 / (self.tt_matrix_num_columns + matrix_num_rows)
            logger.info(
        ...
        logger.info(f"{self.spec.bond_sizes=}, {prod_of_ranks=}, {var_of_cores_elements=}")
    ...
    dimension of a core to determine std of gaussian noise added to cores."""
        ...
        assert all(
            bond_size == self.spec.bond_sizes[1] for bond_size in self.spec.bond_sizes[1:]
        )
        ...
        truncated_scaled_identity = einops.rearrange(
            truncated_scaled_identity,
            "l r -> () l r " + " ".join(("()" for _ in range(self.spec.in_num_channels))),
        )
    ...
    def squared_fro_norm(self) -> torch.Tensor:
        ...
        return contract(
            *chain.from_iterable(
                (core, dim_names)
                for core, dim_names in zip(
                    self.cores, self.spec.get_all_dim_names_add_suffix_to_bonds("_a")
                )
            ),
            *chain.from_iterable(
                (core, dim_names)
                for core, dim_names in zip(
                    self.cores, self.spec.get_all_dim_names_add_suffix_to_bonds("_b")
                )
            ),
    ...
    correct way."""
        ...
        tensor = tensor.reshape(
            *(
                self.spec.in_quantum_dim_size
                for i in range(self.spec.in_num_channels * len(self.spec))
            ),
            -1,
        )
    ...
    def forward(self, input: Union[torch.Tensor, Tuple[torch.Tensor, ...]], /) -> torch.Tensor:
        ...
        aligned_inputs: Iterable[torch.Tensor] = align_with_positions(
            input, self.spec.positions
        )
        ...
        output_tt_cores: Iterable[torch.Tensor] = (
            contract(
                *chain.from_iterable(
                ...
                core,
                core_shape.dimensions_names,
                ("batch", "height", "width", "out_quantum", "bond_left", "bond_right"),
            )
            for core, core_shape, channels in zip(
                self.cores, self.spec.shapes, chunked(aligned_inputs, num_channels)
            )
        )
        output = contract(
            *chain.from_iterable(
                (
                    ...
                    (
                        ...
                        f"out_quantum_{c}",
                        f"bond_{c}",
                        f"bond_{c+1 if c<len(self.cores)-1 else 0}",
                    ),
                )
            ...
            (
                ...
                "height",
                "width",
                *(f"out_quantum_{c}" for c in range(len(self.cores))),
            ),
        )
