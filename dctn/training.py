...
def train(dl: DataLoader, model, optimizer, dev, loss_fn,
          at_iter_start, after_back, after_param_upd
) -> Tuple[StX, StIt]:
  ...
  st_x = {"model": model.to(dev), "optimizer": optimizer, "loss_fn": loss_fn,
          "at_iter_start": list(at_iter_start), "after_back": list(after_back),
          "after_param_upd": list(after_param_upd), "dev": dev}
  ...
  for num_iters_done, (x, y, indices) in enumerate(batches_forever(dl)):
    st_it = {"num_iters_done": num_iters_done, "x": x.to(st_x["dev"]), "y": y.to(st_x["dev"]),
             "indices": indices.to(st_x["dev"]), "stop": False}
...
def every_n_iters_intervals(*intervals):
  ...
  def decorate(func: Callable[[StX, StIt], None]) -> Callable[[StX, StIt], None]:
    def wrapped_func(st_x: StX, st_it: StIt) -> None:
      freq = last(freq for start, (_, freq)
                       in zip(intervals_starts, intervals) if st_it["num_iters_done"] >= start)
...
class BestModelCheckpointer(Checkpointer):
  ...
  def __call__(self, st_x: StX, st_it: StIt):
    ...
    if self.low_is_good and value < self.best_value or not self.low_is_good and value > self.best_value:
      nitd = st_it["num_iters_done"]
      tracc = st_it["train_acc"]
      vacc = st_it["val_acc"]
      trmce = st_it["train_mean_ce"]
...
class ValuesNotImprovingEarlyStopper:
  def __init__(self, patience: int, keys: Tuple[Tuple[str, bool], ...]):
    ...
    self.keys = keys
    self.best_values = [float("+inf") if low_is_good else float("-inf") for _, low_is_good in keys]
